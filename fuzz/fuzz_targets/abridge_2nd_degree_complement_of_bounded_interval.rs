...
fuzz_target!(|bounded_interval: BoundedAbsoluteInterval| {
    let (left_complement, right_complement) = bounded_interval
        .complement()
        .split()
        .expect("The complement of a bounded interval always results in a split complement");

    let left_2nd_degree_complement = left_complement
        .complement()
        .single()
        .expect("The complement of a half-bounded interval always results in a single complement");
