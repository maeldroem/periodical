...
fuzz_target!(|data: (HalfOpenAbsoluteInterval, OverlapRuleSet)| {
    ...
    match (source_half_open.opening_direction(), source_half_open.reference_time_inclusivity(), rule_set) {
        (
            ...
            assert_eq!(overlap_position, DisambiguatedOverlapPosition::OutsideAfter);
        },
        (OpeningDirection::ToFuture, ..) => {
            assert_eq!(overlap_position, DisambiguatedOverlapPosition::OnStart);
        },
        (
            ...
            assert_eq!(overlap_position, DisambiguatedOverlapPosition::OutsideBefore);
        },
        (OpeningDirection::ToPast, ..) => {
            assert_eq!(overlap_position, DisambiguatedOverlapPosition::OnEnd);
        },
    }
});
