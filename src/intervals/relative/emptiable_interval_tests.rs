...
fn from_range() {
    ...
    assert_eq!(
        ...
        HalfBoundedRelativeInterval::new(start, OpeningDirection::ToFuture).to_emptiable_interval()
    );
}
...
fn from_interval() {
    let interval =
        HalfBoundedRelativeInterval::new(SignedDuration::from_hours(1), OpeningDirection::ToFuture).to_interval();
