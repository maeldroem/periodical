...
impl BoundedAbsoluteInterval {
    ...
    /// Creates a new [`BoundedAbsoluteInterval`] of the current [`OffsetIsoWeek`] in the given timezone
    ///
    /// This method requires an offset to use for the [`OffsetIsoWeek`]. Use [`OffsetIsoWeek::ISO_OFFSET`]
    /// if you want regular ISO weeks.
    ///
    /// # Errors
    /// 
    /// Returns [`ComputationError`](BoundedAbsoluteIntervalCreationError::ComputationError) if the given
    /// week start offset is not within the range that [`OffsetIsoWeek`] expects.
    ///
    /// Returns [`OutOfRangeStart`](BoundedAbsoluteIntervalCreationError::OutOfRangeStart) if the week's start
    /// is out of range.
    ///
    /// Returns [`OutOfRangeEnd`](BoundedAbsoluteIntervalCreationError::OutOfRangeEnd) if the week's end
    /// is out of range.
    ///
    /// Returns any error that [`from_inclusive_date_range`](BoundedAbsoluteInterval::from_inclusive_date_range)
    /// may return.
    ...
    ///     OffsetIsoWeek::ISO_OFFSET,
    ///     TimeZone::get("Europe/Oslo")?,
    /// )?;
    /// # Ok::<(), Box<dyn Error>>(())
    /// ```
    pub fn this_week(week_start_offset: i8, tz: TimeZone) -> Result<Self, BoundedAbsoluteIntervalCreationError> {
        let today_iso_week_date = date_today(tz.clone()).iso_week_date();
        let this_week = OffsetIsoWeek::new_with_offset(
            u8::try_from(today_iso_week_date.week())
                .or(Err(BoundedAbsoluteIntervalCreationError::ComputationError))?,
            today_iso_week_date.year(),
            week_start_offset,
        ).or(Err(BoundedAbsoluteIntervalCreationError::ComputationError))?;

        Self::from_inclusive_date_range(
            this_week.first_day().or(Err(BoundedAbsoluteIntervalCreationError::OutOfRangeStart))?,
            this_week.last_day().or(Err(BoundedAbsoluteIntervalCreationError::OutOfRangeEnd))?,
            tz,
        )
    }

    /// Creates a new [`BoundedAbsoluteInterval`] of the next [`OffsetIsoWeek`] in the given timezone
    ///
    /// This method requires an offset to use for the [`OffsetIsoWeek`]. Use [`OffsetIsoWeek::ISO_OFFSET`]
    /// if you want regular ISO weeks.
    ///
    ...
    /// # Ok::<(), Box<dyn Error>>(())
    /// ```
    pub fn next_week(week_start_offset: i8, tz: TimeZone) -> Result<Self, BoundedAbsoluteIntervalCreationError> {
        let next_week_iso_week_date = date_today(tz.clone())
            .checked_add(Span::new().weeks(1))
            .or(Err(BoundedAbsoluteIntervalCreationError::OutOfRangeStart))?
            .iso_week_date();
        let next_week = OffsetIsoWeek::new_with_offset(
            u8::try_from(next_week_iso_week_date.week())
                .or(Err(BoundedAbsoluteIntervalCreationError::ComputationError))?,
            next_week_iso_week_date.year(),
            week_start_offset,
        ).or(Err(BoundedAbsoluteIntervalCreationError::ComputationError))?;

        Self::from_inclusive_date_range(
            next_week.first_day().or(Err(BoundedAbsoluteIntervalCreationError::OutOfRangeStart))?,
            next_week.last_day().or(Err(BoundedAbsoluteIntervalCreationError::OutOfRangeEnd))?,
            tz,
        )
    }

    /// Creates a new [`BoundedAbsoluteInterval`] of the previous [`OffsetIsoWeek`] in the given timezone
    ///
    /// This method requires an offset to use for the [`OffsetIsoWeek`]. Use [`OffsetIsoWeek::ISO_OFFSET`]
    /// if you want regular ISO weeks.
    ///
    ...
    /// # Ok::<(), Box<dyn Error>>(())
    /// ```
    pub fn previous_week(week_start_offset: i8, tz: TimeZone) -> Result<Self, BoundedAbsoluteIntervalCreationError> {
        let next_week_iso_week_date = date_today(tz.clone())
            .checked_add(Span::new().weeks(1))
            .or(Err(BoundedAbsoluteIntervalCreationError::OutOfRangeStart))?
            .iso_week_date();
        let previous_week = OffsetIsoWeek::new_with_offset(
            u8::try_from(next_week_iso_week_date.week())
                .or(Err(BoundedAbsoluteIntervalCreationError::ComputationError))?,
            next_week_iso_week_date.year(),
            week_start_offset,
        ...
        Self::from_inclusive_date_range(
            previous_week.first_day().or(Err(BoundedAbsoluteIntervalCreationError::OutOfRangeStart))?,
            previous_week.last_day().or(Err(BoundedAbsoluteIntervalCreationError::OutOfRangeEnd))?,
            tz,
        )
    }
